  ...
  <div>
    <div id="kanbanutama" class="d-flex container mt-3">
      <div id="mainkanban" class="utama d-flex flex-column">
        <h6 class="mt-1">BACKLOG</h6>
        <div v-for="(data,idx) in backlogdata" class="flex-column col bg-dark my-1 rounded" :key="data.id">
          ...
          <div>
            ...
            <modal height="auto" :scrollable="true" name="edittaskbacklog">
...
export default {
  ...
  computed: {
    backlogdata() {
      return this.task.filter(filter => filter.category == "backlog");
    }
  },
    data() {
    return {
