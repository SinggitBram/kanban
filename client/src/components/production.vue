  ...
  <div>
    <div id="kanbanutama" class="d-flex container mt-3">
      <div id="mainkanban" class="utama d-flex flex-column">
        <h6 class="mt-1">PRODUCTION</h6>
        <div v-for="(data,idx) in productiondata" class="flex-column col bg-dark my-1 rounded" :key="data.id">
          ...
          <div>
            ...
            <modal height="auto" :scrollable="true" name="edittaskproduction">
...
export default {
  ...
  computed: {
    productiondata() {
      return this.task.filter(filter => filter.category == "production");
    }
  },
    data() {
    return {
